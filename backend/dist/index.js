...
const emailService_1 = require("./emailService"); // Adjust path if needed
const crypto_1 = __importDefault(require("crypto"));
const multer_1 = __importDefault(require("multer"));
const path_1 = __importDefault(require("path"));
const uuid_1 = require("uuid");
// NEW: Import the Redis client library
const redis_1 = require("redis");
app.use(express_1.default.json());
app.use(express_1.default.urlencoded({ extended: true }));
app.use((0, cors_1.default)());
// NEW: Set up and connect the Redis client for the token blocklist
const redisClient = (0, redis_1.createClient)({
// By default, it connects to redis://127.0.0.1:6379
// For production, use a URL from an environment variable:
// url: process.env.REDIS_URL
});
redisClient.on('error', (err) => console.log('Redis Client Error', err));
// Use an immediately invoked async function to connect
(async () => {
    await redisClient.connect();
    console.log("Connected to Redis successfully!");
})();
// NEW: This middleware is updated to check the Redis blocklist.
...
app.post("/api/auth/register", async (req, res) => {
    try {
        ...
        // Check for existing user by either email OR username in one query
...
app.post("/api/auth/login", async (req, res) => {
    try {
        // 1. Destructure email, username, and password from the request body.
        ...
        // 2. Use findFirst with an 'OR' clause to find the user.
        ...
        // IMPORTANT: Replace "YOUR_SECRET_KEY" with a real secret from a .env file
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, // You can add more user data to the payload if needed
...
app.post("/api/auth/logout", authenticateToken, async (req, res) => {
    try {
        ...
        // The middleware already confirms the token is valid, but we double-check
        if (!token || !req.user) {
...
app.post("/api/auth/request-password-reset", async (req, res) => {
    try {
        ...
        // Always send a success-like message to prevent email enumeration.
        if (!user) {
        ...
        await (0, emailService_1.sendPasswordResetOtp)(user.email, otp);
...
app.post("/api/auth/reset-password", async (req, res) => {
    try {
        ...
        const isOtpValid = await bcryptjs_1.default.compare(otp, validOtp.otpCode);
        if (!isOtpValid) {
            return res.status(400).json({ error: "Invalid OTP or email" });
        }
...
// Add this new endpoint to your index.ts
// It must be protected by your authenticateToken middleware.
app.get("/api/users/me", authenticateToken, async (req, res) => {
    try {
        ...
        // Use Prisma's `select` to pick fields and `_count` for relations
        ...
        // Restructure the response to be cleaner (e.g., flatten the _count object)
        const response = {
            id: userProfile.id,
...
app.patch("/api/users/me", authenticateToken, async (req, res) => {
    try {
        ...
        const updatedUser = await prisma.user.update({
            ...
            // 3. CRITICAL: Select only the safe fields to return to the client.
            // This prevents leaking the passwordHash or other sensitive data.
            select: {
                id: true,
                username: true,
                email: true,
...
//all the post related endpoints
...
// Optional: File filter to accept only certain image types
...
const upload = (0, multer_1.default)({ storage: storage, fileFilter: fileFilter, limits: { fileSize: 1024 * 1024 * 5 } }); // Limit file size to 5MB
// --- The Endpoint ---
...
app.get("/api/posts/:postId/comments", async (req, res) => {
    try {
        ...
        // --- Pagination Logic ---
        const page = parseInt(req.query.page) || 1;
        const pageSize = parseInt(req.query.pageSize) || 15; // A larger page size might be suitable for comments
        const skip = (page - 1) * pageSize;
...
app.delete("/api/comments/:commentId", authenticateToken, async (req, res) => {
    try {
        ...
        if (!(0, uuid_1.validate)(commentId)) {
...
app.post("/api/users/:userId/follow", authenticateToken, async (req, res) => {
    try {
        ...
        if (!(0, uuid_1.validate)(followingId)) {
...
app.delete("/api/admin/users/:userId", authorizeAdmin, async (req, res) => {
    try {
        ...
        // 1. Validate the User ID format
        if (!(0, uuid_1.validate)(userIdToDelete)) {
            return res.status(400).json({ error: "Invalid User ID format." });
        }
...
app.delete("/api/admin/posts/:postId", authorizeAdmin, async (req, res) => {
    try {
        const { postId } = req.params;
        // 1. Validate the Post ID format
        if (!(0, uuid_1.validate)(postId)) {
...
app.delete("/api/admin/comments/:commentId", authorizeAdmin, async (req, res) => {
    try {
        const { commentId } = req.params;
        // 1. Validate the Comment ID format
        if (!(0, uuid_1.validate)(commentId)) {
